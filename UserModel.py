...
import users
user = users.User()
user.create("john@greenanimalsbank.com", "password")
user.update("john@greenanimalsbank.com", password="new password", role=users.Role.admin)
print(user.role("john@greenanimalsbank.com"))
> "admin"
...
class User(object):
    ...
    def role(self, email):
        ...
        Example:
        > user = User()
        > print(user.role("test@greenanimalsbank.com")==user.Role.admin)
        """
...
def test_user():
    ...
    user = User()
    assert user.create("test@greenanimalsbank.com", "test")==1
    assert user.role("test@greenanimalsbank.com")==Role.user
    assert user.authenticate("test@greenanimalsbank.com", "test") is True
    assert user.canAnnotate("test@greenanimalsbank.com") is False
    assert user.update("test@greenanimalsbank.com", "", "annotator")==1
    assert user.canAnnotate("test@greenanimalsbank.com") is True
    assert user.update("test@greenanimalsbank.com", "new password", "annotator")==1
    assert user.authenticate("test@greenanimalsbank.com", "test") is False
    assert user.authenticate("test@greenanimalsbank.com", "new password") is True
    assert user.delete("test@greenanimalsbank.com")==1
